...
def create_model_service(
  ...
  class ModelService:
    ...
    async def item_update(
      ...
      # 根据id查询要更新的记录
      update_cls = (await session.execute(self.select_cls().where(Cls.id == row_dict.get('id')))).scalars().first()
      if not update_cls:
        # 若记录不存在，抛出异常
    ...
    async def batch_update(
      ...
      # 根据id查询所有待更新的记录
      update_cls_list = (await session.execute(self.select_cls().where(Cls.id.in_(update_id_list)))).all()
    ...
    async def item_delete(self, session: AsyncSessionDep, row_dict: dict = Body(..., description=f"删除的数据，字段参考{Cls.__name__}")):
      ...
      # 根据id查询要删除的记录
      delete_cls = (await session.execute(self.select_cls().where(Cls.id == row_dict.get('id')))).scalars().first()
      if not delete_cls:
        # 若记录不存在，返回删除失败
        return False
    ...
    async def batch_delete(self, session: AsyncSessionDep, row_dict_list: List[dict] = Body(..., description=f"批量删除的数据数组，字段参考{Cls.__name__}")):
      ...
      # 根据id查询所有待删除的记录
      delete_cls_list = (await session.execute(self.select_cls().where(Cls.id.in_(row_id_list)))).all()
