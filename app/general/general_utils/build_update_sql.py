...
from app.general.general_utils.sql_utils import get_value, format_columns, get_value_sql, log_sql
...
def build_update_sql(module_config, row, param_update_fields=None):
  ...
  for hump_name, column in column_info['hump_to_columns'].items():
    ...
    if not column['query'].startswith('t1.'):
      continue
    if hump_name != 'updateAt' and param_update_fields is not None and hump_name not in param_update_fields:
      # 如果有指定更新的字段，并且humpName不在这个字段列表中，则不更新这个字段
      continue
    if hump_name == 'updateAt':
      value = datetime.now().strftime('%%Y-%%m-%%d %H:%M:%S')
    field_sql_list.append(f"{column['col_name']} = {get_value_sql(value=value, value_type=column['valueType'], sql_values=values, )}")
